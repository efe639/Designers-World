...
module.exports = {
    name: "Kışkırtma, Trol",
    description: " Kışkırtma, Trol, Dalgacı ve Ortam Bozucu Davranı - 10 Dk",
    type: 'MESSAGE',
  /**
     *
     * @param {Client} client
    ...
    run: async (client, interaction,args) => {
      ...
      const member = message.member
       ...
       if (susturmasüresi > şimdikizaman) return  interaction.followUp({content:`${emojilrt.uyari} Susturmak istediğiniz kullanıcı zaten susturulmuş gözüküyor.`, ephemeral: true });
        ...
        await interaction.guild.channels.cache.get("980429446437797898").send({embeds:[embed.setDescription(`${member}, ${interaction.member} (\`${interaction.member.user.username}\`) tarafından **Kışkırtma, Trol, Dalgacı ve Ortam Bozucu Davranı** sebebiyle ${turkishDate(sure)} boyunca zaman aşımına maruz bırakıldı.`)]})
        await interaction.followUp({content:`${member}, ${interaction.member} tarafından **Kışkırtma, Trol, Dalgacı ve Ortam Bozucu Davranı** sebebiyle ${turkishDate(sure)} boyunca ses ve yazılı kanallarda susturuldu.`})
    },
};