...
module.exports = {
    name: "Dini, Irki, ve Siyasi",
    description: "Dini, Irki, ve Siyasi Değerlere Hakaret - 30 Dk",
    type: 'MESSAGE',
  /**
     *
     * @param {Client} client
    ...
    run: async (client, interaction,args) => {
      ...
      const member = message.member
       ...
       if (susturmasüresi > şimdikizaman) return  interaction.followUp({content:`${emojilrt.uyari} Susturmak istediğiniz kullanıcı zaten susturulmuş gözüküyor.`, ephemeral: true });
        ...
        await interaction.guild.channels.cache.get("980429446437797898").send({embeds:[embed.setDescription(`${member}, ${interaction.member} (\`${interaction.member.user.username}\`) tarafından **Dini, Irki, ve Siyasi Değerlere Hakaret** sebebiyle ${turkishDate(sure)} boyunca zaman aşımına maruz bırakıldı.`)]})
        await interaction.followUp({content:`${member}, ${interaction.member} tarafından **Dini, Irki, ve Siyasi Değerlere Hakaret** sebebiyle ${turkishDate(sure)} boyunca ses ve yazılı kanallarda susturuldu.`})
    },
};